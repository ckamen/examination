 ...
 */
function getMyExamList(listPage, page, limit, sort, order, q, catalogId, compulsoryType) {
  //1.检查参数
  page = ("undefined" == typeof page) ? 1 : page;
  limit = ("undefined" == typeof limit) ? 5 : limit;
  ...
  wx.request({
    url: app.globalData.commonURI + app.globalData.myExamListURI,
    data: {
      page: page,
      limit: limit,
      sort: sort,
    ...
    success: function (res) {
      ...
      if (1 == code) {//获取考试列表成功
        ...
        wx.setStorageSync(app.globalData.nextPage, '/pages/mylist/mylist');//指示待登录成功后要实际跳转的页面
        wx.navigateTo({
          url: '/pages/login/login'
        });
      } else {//0:执行产生错误
        util.wxShowModal('访问失败', '获取我的考试列表失败:' + res.data.message);
      }
    },
    fail: function () {
